...
class MealOfferManager {
    ...
    public static function insertMealOffer($meal_offer){
        ...
        $db_instance->connect();
        $sql_insert = $db_instance->prepare("INSERT INTO meal_offer (meal_type, meal_name, continent,country,description,min_seat, max_seat, price,date,start_time, course_option, end_time,donation_type,number_of_donations) "
                . "VALUES(?, ?, ?,?, ?, ?,?, ?, ?,?, ?, ?,?, ?);");
        ...
        $sql_insert->bind_param("ssiisiidssissi", $meal_type, $meal_name, $continent, $country, $description, $min_seat, $max_seat, $price, $date, $start_time, $course_option, $end_time, $donation_type, $number_of_donations);
        $db_instance->executeStatement();
        $db_instance->closeStatement();
        $db_instance->closeConnection();
    }
    
    public static function deleteMenu($meal_offer) {
        $db_instance = DBManager::getInstance();
    ...
    public static function fetchMealOffer(){
        ...
        $tupple = null;
        $statement->bind_result($meal_type, $meal_name, $continent, $country, $description, $min_seat, $max_seat, $price, $date, $start_time, $course_option, $end_time, $donation_type, $number_of_donations);
        $db_instance->executeStatement();
        
        while($db_instance->fetchResult()){
            $tupple = new MealOffer(null,$meal_type, $meal_name, $continent, $country, $description, $min_seat, $max_seat, $price, $date, $start_time, $course_option, $end_time, $donation_type, $number_of_donations);
